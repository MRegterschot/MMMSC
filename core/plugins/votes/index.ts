...
/*    async onStart() {
    ...
    async updateWidget(login: string, yes: number = 0, no: number = 0, total: number = 0, percent: number = 0) {
        ...
        widget.setData({
            ...
            total,
            yes_ratio: percent,
            voteText: `${(await tmc.players.getPlayer(this.currentVote.starter)).nickname} asks ${processColorString(htmlEntities(this.currentVote.question))}`,
